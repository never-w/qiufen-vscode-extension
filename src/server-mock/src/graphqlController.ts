...
import { getOperationsBySchema /* removeMockDirectivesFromDocument */ } from "@fruits-chain/qiufen-helpers"
// import { Kind, parse } from "graphql"
import { addMocksToSchema } from "./addMocksToSchema"
import type { /* BooleanValueNode */ GraphQLSchema /* OperationDefinitionNode */ } from "graphql"
...
const createGraphqlController = async (config: GraphqlKitConfig, rawSchema: GraphQLSchema) => {
  ...
  //       noLocation: true,
  //     }).definitions.find((item) => item.kind === Kind.OPERATION_DEFINITION) as OperationDefinitionNode | undefined
  //     return (operation?.directives?.find((item) => item.name.value === "mock")?.arguments?.find((arg) => arg.name.value === "enable")?.value as BooleanValueNode | undefined)
  //       ?.value
  //   }
  //   const isIntrospectionQuery = req.body.operationName === "IntrospectionQuery"
  //   if ((mock?.enable && (operationMockDirectiveEnable() ?? mock.mockDirectiveDefaultEnableValue ?? true)) || isIntrospectionQuery) {
  //     console.log(chalk.red(`[${req.socket.remoteAddress}]`, dayjs().format("HH:mm:ss"), "target: mock ", req.body.operationName))
  //     next()
  //   } else {
  //     console.log(chalk.red(`[${req.socket.remoteAddress}]`, dayjs().format("HH:mm:ss"), "target: backend ", req.body.operationName))
