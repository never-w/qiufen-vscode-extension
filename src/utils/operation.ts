...
function _normalizeGraphqlOutputType(type: GraphQLOutputType, refChain: string[] = [], scalarMap: ScalarMap = {}): OutputType {
  const namedType = getNamedType(type)
  const typeName = type.toString()
  const ofTypeName = namedType.name
 ...
 */
export function getOperationsBySchema(schema: GraphQLSchema, scalarMap: ScalarMap = {}): TypedOperation[] {
  return [
 ...
 */
export function groupOperations(operations: TypedOperation[], groupBy: GroupByFn = _groupBy) {
