...
function hasConflictingVariableDefinitions(
  localVars: ReadonlyArray<VariableDefinitionNode>,
  remoteVars: ReadonlyArray<VariableDefinitionNode>
): boolean {
...
function updateAst(localNode: DefinitionNode, remoteNode: DefinitionNode): DefinitionNode | null {
  ...
  if (localNode.kind === 'ObjectTypeDefinition' && remoteNode.kind === 'ObjectTypeDefinition') {
    ...
    localObjectTypeNode.fields = localObjectTypeNode.fields
      .map((localField) => {
        const remoteField = remoteObjectTypeNode.fields.find(
          (remoteField) => remoteField.name.value === localField.name.value
        );
        if (!remoteField) {
          return null;
        }
  ...
  if (localNode.kind === 'InterfaceTypeDefinition' && remoteNode.kind === 'InterfaceTypeDefinition') {
    ...
    localInterfaceNode.fields = localInterfaceNode.fields
      .map((localField) => {
        const remoteField = remoteInterfaceNode.fields.find(
          (remoteField) => remoteField.name.value === localField.name.value
        );
        if (!remoteField) {
          return null;
  ...
  if (localNode.kind === 'OperationDefinition' && remoteNode.kind === 'OperationDefinition') {
    ...
    if (hasConflictingVariableDefinitions(localOperationNode.variableDefinitions, remoteOperationNode.variableDefinitions)) {
      remoteOperationNode.variableDefinitions.forEach((remoteVar) => {
        const localVar = localOperationNode.variableDefinitions.find(
          (localVar) => localVar.variable.name.value === remoteVar.variable.name.value
        );
        if (!localVar) {
...
query inOrder($pageCostOrderInput: PageCostOrderInput, $page: Page) {
   inOrder(pageCostOrderInput: $pageCostOrderInput, page: $page) {
    pageCurrent
    pageSize
    totalRecords
    records {
