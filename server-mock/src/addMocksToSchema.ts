...
export function addMocksToSchema({ schema, scalarMap, resolvers, globalContext = {} }: IOptions): GraphQLSchema {
  const mockResolver: GraphQLFieldResolver<unknown, any> = (source, args, context, info) => {
    const variables = context.body.variables
    const mockDirective = info.fieldNodes[0].directives?.find((item) => item.name.value === "mock")
    const mockErr = mockDirective?.arguments?.find((item) => item.name.value === "err")
    if (mockErr) {
      const errFields = (mockErr.value as ObjectValueNode).fields
      const code = (errFields.find((item) => item.name.value === "code")?.value as IntValueNode).value
      const message = (errFields.find((item) => item.name.value === "message")?.value as StringValueNode).value
    ...
    const genMockVal = (val: ArgumentNode, path: GraphQLResolveInfo["path"]) => {
      switch (val.value.kind) {
        ...
        case Kind.STRING:
          return interpret(val?.value.value, Object.assign({ ...globalContext }, variables, { path }))
        case Kind.NULL:
          return null
        default:
    ...
    const mockData = (type: GraphQLOutputType, path: GraphQLResolveInfo["path"], len = 2): unknown => {
  ...
  const schemaWithMocks = mapSchema(schema, {
    [MapperKind.OBJECT_FIELD]: ({ ...fieldConfig }) => {
      fieldConfig.resolve = async (source, args, context, info) => {
        const [mockedValue, resolvedValue] = await Promise.all([mockResolver(source, args, context, info), defaultFieldResolver(source, args, context, info)])
